...
QuadTree.prototype.getChild = function(quadrant){
	if(quadrant === "NE"){
		return this.children["NE"];
	}
};

QuadTree.prototype.insertNode = function(quadrant){
	if(quadrant === "NE"){
		var leftUp = this.leftUp.clone();
		var rightUp = this.rightUp.clone();
		var centerPoint = this.centerPoint.clone();
		var leftDown = this.leftDown.clone();
		var rightDown = this.rightDown.clone();

		var upper = new THREE.Vector3();
		upper = upper.subVectors(rightUp, leftUp);

		var newLeftUp = leftUp.add(upper.divideScalar(2));
		var newRightUp = rightUp.clone();

		var newLeftDown = centerPoint.clone();
		var rightEdge = new THREE.Vector3();
		rightEdge = rightEdge.subVectors(rightDown, rightUp);

		var newRightDown = rightUp.add(rightEdge.divideScalar(2));

		var diagonalV = new THREE.Vector3();
		diagonalV = diagonalV.subVectors(newRightDown, newLeftUp);

		var temp = newLeftUp.clone()

		var newCenter = temp.add(diagonalV.divideScalar(2));
		...
		this.children["NE"] = new QuadTree(tempGeometry);
	}
};*/
